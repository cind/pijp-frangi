...
df = pd.DataFrame(data=zip(subject_codes,researchgroup,pvsvol,pvscount,icvnorm_vol,pvsvolwm,pvscountwm,icvnorm_volwm),index=np.arange(len(subject_codes))+1,columns=col)
df .to_csv(os.path.join(get_project_dir(basics.project),'frangidata.csv'), index=True)
